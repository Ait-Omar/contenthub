...
const TableOfContents: React.FC<{ activeId: string }> = ({ activeId }) => {
    ...
    return (
        <nav className="p-4 bg-white/50 dark:bg-gray-800/50 rounded-lg backdrop-blur-sm border border-gray-200 dark:border-gray-700">
            <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 mb-3">On this page</h3>
...
const SubSection: React.FC<{ id: string; title: string; children: React.ReactNode; refProp: React.RefObject<HTMLElement> }> = ({ id, title, children, refProp }) => (
     <section id={id} ref={refProp} className="scroll-mt-20 pt-4">
            ...
            <header className="fixed top-0 left-0 w-full z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border-b border-gray-200 dark:border-gray-700/50 transition-colors duration-300">
                <div className="container mx-auto flex justify-between items-center h-16 px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center">
                        ...
                        <span className="hidden sm:inline text-xl font-bold text-gray-800 dark:text-gray-200 ml-2">ContentHub Guide</span>
                    </div>
                    <button onClick={onBack} className="text-sm font-semibold py-2 px-4 bg-primary text-white rounded-md shadow-sm hover:bg-primary-dark transition-colors">
                        &larr; Back to Home
                    </button>
                </div>
            </header>
